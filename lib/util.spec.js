...
test('util.getEntriesQuery - it should replace TODAY with the current date', t => {
  let query = util.getEntriesQuery({ filter: { 'startdate[gt]': 'TODAY' } })
  let now = new Date()

  // there's gonna be a gap here
  if (query['startdate[gt]'].startsWith(now.toISOString().substr(0, 14))) {
    t.pass('date properly generated')
  } else {
    t.fail('date not properly generated')
  }
})
